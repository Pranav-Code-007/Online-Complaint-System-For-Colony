...
class LoginForm(FlaskForm):
    email = StringField('Email',
                        validators=[DataRequired(), Length(min=10, max=25), Email()],
                        render_kw={"placeholder": "Email"})
    password = PasswordField('Password', validators=[DataRequired(), Length(min=8, max=12)],
                             render_kw={"placeholder": "Password"})
    choices = ['Citizen', 'Councillor']
    choice = SelectField('Role :', choices=choices, validators=[DataRequired()])
    submit = SubmitField('Login')


class ComplaintForm(FlaskForm):
    complaint = TextAreaField('Write Here', validators=[DataRequired(), Length(min=12, max=150)],
                              render_kw={"placeholder": "Write here upto 150 characters"})
    subject = TextAreaField('Subject', validators=[DataRequired(), Length(min=3, max=20)],
                              render_kw={"placeholder": "Write here upto 20 characters"})
    submit = SubmitField('Submit Complaint')

class Submit(FlaskForm):
    submit = SubmitField('Delete All Complaints')